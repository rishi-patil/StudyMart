...
const CourseCard = ({ cardData, currCard, setCurrCard }) => {
  return (
    ...
    ${currCard === cardData.heading ? "bg-white shadow-[12px_12px_0_0] shadow-yellow-50" : "bg-richblack-800"}
    `}
      onClick={() => setCurrCard(cardData.heading)}
    >
      <div className="border-b-[2px] border-richblack-400 border-dashed h-[80%] p-6 flex flex-col gap-3">
          <div className = {` ${currCard === cardData.heading && "text-richblack-800"} font-semibold text-[20px]`} > {cardData?.heading} </div>
          <div className="text-richblack-400">{cardData.description}</div>
        </div>

        <div className = {`flex justify-between ${currCard === cardData.heading ? "text-blue-300" : "text-richblack-300" } px-6 py-3 font-medium`} >  
